...
var sql = {
  ...
  escapeId: function (val) {
    return "[%s]".replace('%s', val.replace(/'/g, "''"));
  },
...
function toSqlDate(date) {
  return 'YYYY-MM-DDThh:mm:ss.nn'
    .replace('YYYY', date.getUTCFullYear())
    .replace('MM', ('00' + date.getUTCMonth() + 1).slice(-2))
    .replace('DD', ('00' + date.getUTCDate()).slice(-2))
    .replace('hh', ('00' + date.getUTCHours()).slice(-2))
    .replace('mm', ('00' + date.getUTCMinutes()).slice(-2))
    .replace('ss', ('00' + date.getUTCSeconds()).slice(-2))
    .replace('nn', date.getUTCMilliseconds());
}
