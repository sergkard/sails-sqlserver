...
var sql = {
  ...
  addColumn: function (collectionName, attrName, attrDef) {
    ...
    return 'ALTER TABLE [' + require('../../../config').dbSchema + '].[' + tableName + '] ADD ' + columnDefinition;
  },
  ...
  removeColumn: function (collectionName, attrName) {
    ...
    return 'ALTER TABLE [' + require('../../../config').dbSchema + '].[' + tableName + '] DROP COLUMN ' + attrName;
  },
  ...
  insertQuery: function (collectionName, data) {
    ...
    return 'DECLARE @Inserted AS TABLE (id NVARCHAR(255)); INSERT INTO [' + require('../../../config').dbSchema + '].[' + tableName + '] ' + '(' + sql.attributes(collectionName, data) + ')' + 'OUTPUT INSERTED.[id] INTO @Inserted VALUES (' + sql.values(collectionName, data) + '); SELECT id FROM @Inserted';
  },
