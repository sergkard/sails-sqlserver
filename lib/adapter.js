  ...
  var adapter = {
    ...
    registerConnection: function (connection, collections, callback) {
      if (!connection.identity) {
        return callback(new Error("Connection is missing an identity."));
      }

      if (connections[connection.identity]) {
        return callback(new Error("Connection is already registered."));
      }
     ...
     * @param {Function} cb
     */
    connectConnection: function (connection, callback) {
      ...
      if (isPersistent && (!connections[connection].mssqlConnection || !connections[connection].mssqlConnection.connected)) {
        ...
        connections[connection].mssqlConnection.connect().then(callback);
        ...
        connections[connection].mssqlConnection[uniqId].connect().then(function (err) {
          callback(err, uniqId);
        });
      } else {
        _.defer(callback);
    ...
    teardown: function (conn, callback) {
    ...
    // Return attributes
    describe: function (connection, collection, callback) {
      ...
      adapter.connectConnection(connection, function __DESCRIBE__(err, uniqId) {
        ...
        request.query(statement, function (err, recordset) {
          ...
          callback(null, normalizedSchema);
        });
      });
    },
     ...
     *
     */
    drop: function (connection, collection, relations, callback) {
    ...
    // Raw Query Interface
    query: function (connection, collection, query, data, callback) {
      ...
      adapter.connectConnection(connection, function __UPDATE__(err, uniqId) {
        ...
        request.query(statement, function (err, recordset) {
          ...
          request.query(statement, function (err) {
            ...
            return adapter.find(connection, collection, criteria, callback);
          });
        });
      });
    },
    ...
    join: function (connectionName, collectionName, criteria, callback) {
