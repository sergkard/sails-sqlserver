...
module.exports = (function () {
  ...
  var adapter = {
    ...
    registerConnection: function (connection, collections, cb) {
      if (_.isUndefined(connection.schemaDatabase)) {
        return cb(new Error('Missing schema <schemaDatabase> in adapter parameters'))
      }
    ...
    describe: function (connection, collection, cb) {
      ...
      adapter.connectConnection(connection, function __DESCRIBE__(err, uniqId) {
        ...
        if (process.env.WEBSITE_MODE !== 'production') {
        ...
        request.query(statement, function (err, recordset) {
          ...
          if (connections[connection] && connections[connection].config && !connections[connection].config.persistent && mssqlConnect) {
            mssqlConnect.close();
          }
    ...
    find: function (connection, collection, options, cb) {
      var schema = connections[connection].config.schemaDatabase;
      ...
      var statement = sql.selectQuery(collection, schema, options);
          ...
          for (var i = 0, l = data.length; i < l; i++) {
            if (_.isArray(data[i])) {
              ...
              for (var j=0, length = data[i].length; j < length; j++) {
                dataArray.push('\'' + escape(data[i][j]) + '\'');
              }
              query = query.replace(new RegExp('\\$' + String(i + 1), 'g'), dataArray.toString());
            } else {
    ...
    create: function (connection, collection, values, cb) {
      ...
      const ai = adapter.autoIncremtedPk(connection, collection);
      var schema = connections[connection].config.schemaDatabase;

      Object.keys(values).forEach(function (key) {
        values[key] = utils.prepareValue(values[key]);
        if (process.env.WEBSITE_MODE !== 'production') {
          console.log('pk insert identity', (ai && pk === key), pk, key, ai);
        }
        if (ai && pk === key) {
          identityInsert = true;
        }
      });
      const schemaName = getSchemaName(connection, collection);
      const tableName = '[' + schemaName + ']' + '.[' + collection + ']';
      var statement = sql.insertQuery(collection, schema, values);
      if (identityInsert) {
      ...
      adapter.connectConnection(connection, function __CREATE__(err, uniqId) {
        ...
        request.query(statement, function (err, recordsets) {
          ...
          if (!_.isUndefined(recordsets) && !_.isEmpty(recordsets)) {
            var recordset = recordsets[0];
            if (recordset.id) {
              model = _.extend({}, values, {id: recordset.id});
            }
          }
    ...
    autoIncremtedPk: function (connection, collection) {
      let autoincremented = false;
      Object.keys(connections[connection].collections[collection].definition).forEach(function (key) {
        if (process.env.WEBSITE_MODE !== 'production') {
          console.log('autoIncrementPk', connections[connection].collections[collection].definition[key].autoIncrement);
        }
        if (!connections[connection].collections[collection].definition[key].hasOwnProperty('autoIncrement')) {
          return;
        }

        autoincremented = autoincremented || connections[connection].collections[collection].definition[key].autoIncrement;
      });
    ...
    update: function (connection, collection, options, values, cb) {
      ...
      var statement = 'SELECT [' + pk + '] FROM [' + schema + '].[' + tableName + '] ' + criteria;
      ...
      adapter.connectConnection(connection, function __UPDATE__(err, uniqId) {
        ...
        request.query(statement, function (err, recordset) {
          ...
          var statement = 'UPDATE [' + schema + '].[' + tableName + '] SET ' + sql.updateCriteria(collection, values) + ' ';
          ...
          request.query(statement, function () {
            if (err) {
              return cb(err);
            }
            ...
            if (pks.length === 1) {
              ...
              if (!_.isUndefined(values[pk])) {
                criteria.where[pk] = values[pk];
              }
            } else {
              criteria = {
                where: {}
    ...
    destroy: function (connection, collection, options, cb) {
      var tableName = collection;
      var schema = connections[connection].config.schemaDatabase;
      var statement = 'DELETE FROM [' + schema + '].[' + tableName + '] ';
  ...
  function getSchemaName (connection, collection) {
    const collectionObject = connections[connection].collections[collection];
